...
def do_video_recognition():
    ...
    # cap = cv2.VideoCapture("joined_video.mp4")
    cap = cv2.VideoCapture("rtsp://omoikane:k4m1z4m4@10.73.19.103/stream1")
    while cap.isOpened():
        ret, frame = cap.read()
        if not ret:
            break
        ...
        if faces:
            for face in faces:
                ...
                # identity = isf.FaceIdentity(feature, id=idx)  # array of 512 floats
                # print(f"face: {face}")
                cv2.rectangle(frame, (face.location[0], face.location[1]), (face.location[2], face.location[3]), (0, 0, 255), 4)
        resframe = resize_frame(frame, 500)
...
def case_face_recognition(test_data_folder):
    ...
    for idx, image_path in enumerate(insert_images):
        ...
        if faces:
            ...
            # face: FaceInformation(track_id=-1, track_count=1,
            # detection_confidence=0.5956331491470337, location=(6, 11, 151, 164), roll=0.0,
            # yaw=0.0, pitch=0.0)
            feature = session.face_feature_extract(image, face) # array of 512 floats
            identity = isf.FaceIdentity(feature, id=idx) # array of 512 floats
